...
def train_ddp(rank, world_size, config):
    ...
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = '12355'
    timeout = timedelta(minutes=30)# タイムアウトを30分に設定
    dist.init_process_group("nccl", rank=rank, world_size=world_size, timeout=timeout)
    
    wandb.init(project="chemtrichomod", config=config)
    config = wandb.config
    
    feat_type = config.feat_type

    # データの準備
    hdf5_filename = '/home/gemma/ml/TSenergy/experiment/dataset_40atoms/h5data/data_2013ChemEuroJ_Trichobrasilenol.h5'
    external_hdf5_filename = '/home/gemma/ml/TSenergy/experiment/dataset_40atoms/h5data/testdata_2013ChemEuroJ_Trichobrasilenol.h5'
    ...
    best_model_path = os.path.join(save_dir, f'best_model_rank{rank}.pth')
    ...
    no_improve_count = 0
    last_val_loss = None
    best_val_loss = float('inf')  # ベスト検証損失
    ...
    for epoch in range(config.epochs):
        ...
        # ベストモデルの保存
        if epoch >= 50 and val_loss < best_val_loss:  # 50エポック以降のみ保存
            best_val_loss = val_loss
            if rank == 0:  # 主プロセスのみ保存
                # ファイル名に実行名とエポック番号を追加
                best_model_path = os.path.join(save_dir, f'best_model_{run_name}_epoch{epoch + 1}_rank{rank}.pth')
    ...
    if rank == 0:
        # ファイル名に実行名と最終エポックを追加
        final_model_path = os.path.join(save_dir, f'final_model_{run_name}_epoch{epoch + 1}_rank{rank}.pth')
