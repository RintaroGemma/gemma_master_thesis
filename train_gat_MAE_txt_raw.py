...
def create_pyg_data(atomic_coordinates, parsed_bonds, energy_kcal):
    coordinates = []
    ...
    # ターゲット値（kcal/mol）をそのまま使用
    y = torch.tensor([energy_kcal], dtype=torch.float).view(-1, 1)
    ...
    for input_dir in input_dirs:
        ...
        for file_path in file_paths:
            ...
            for line in energy_info:
                if "Energy: " in line and "kcal/mol" in line:
                    energy_kcal = float(line.split(":")[1].strip().split()[0])
                    break
            
            if energy_kcal is not None:
                pyg_data = create_pyg_data(atomic_coordinates, parsed_bonds, energy_kcal)
...
def weight_init(m):
    ...
    if isinstance(m, torch.nn.Linear) and m.out_features == 1:
        torch.nn.init.uniform_(m.weight, a=-100, b=100)

# wandbを初期化
wandb.init(project="GAT_raw_mod")

# ハイパーパラメータのログ
config = wandb.config
config.hidden_channels = wandb.config.get("hidden_channels", 128)
config.optimizer = wandb.config.get("optimizer", "adam")
config.learning_rate = wandb.config.get("learning_rate", 0.001)  # 学習率を増加
...
# 外部テストデータのロード
external_data_path = '/home/gemma/ml/TSenergy/experiment/dataset_40atoms/energy_normalized'  # 外部テストデータのパスに置き換えてください
